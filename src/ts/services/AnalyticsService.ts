...
class AnalyticsService {
  ...
  async fetchPlatformData(platform: string): Promise<any> {
    try {
  ...
  renderReachChart(data: AnalyticsData): void {
    const ctx = document.getElementById('reachChart') as HTMLCanvasElement;
    if (!ctx) return;
  ...
  updateMetrics(data: AnalyticsData): void {
    ...
    if (followerTotal) followerTotal.textContent = data.followers.total.toLocaleString();
    if (followerChange) {
    ...
    if (totalLikes) totalLikes.textContent = data.engagement.likes.toLocaleString();
    if (totalComments) totalComments.textContent = data.engagement.comments.toLocaleString();
    if (totalShares) totalShares.textContent = data.engagement.shares.toLocaleString();
    ...
    if (reachTotal) reachTotal.textContent = data.reach.total.toLocaleString();
    if (reachChange) {
    ...
    if (totalPosts) totalPosts.textContent = data.posts.total.toString();
    if (scheduledPosts) scheduledPosts.textContent = data.posts.scheduled.toString();
    if (publishedPosts) publishedPosts.textContent = data.posts.published.toString();
  }
